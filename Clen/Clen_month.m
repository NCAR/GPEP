...
cctype='Pearson';
leastyear=20;
outfileCC=['Clen_month_',cctype,'.mat'];
outfileFit=['Fit_month_',cctype,'.mat'];
...
if ~exist(outfileCC,'file')
    ...
    for vv=1:length(vars)
        ...
        % daily to monthly
        datavv2=nan*zeros(length(yyyymmu),size(datavv,2));
        for i=1:length(yyyymmu)
            temp=datavv(yyyymm==yyyymmu(i),:);
            datavv2(i,:)=nanmean(temp,1);
        end
        datavv=datavv2; clear datavv2
        ...
        parfor i=1:gnum-1
            ...
            for j=1:size(llei,1)
                ccij=f_monthmon_cc(datavv(:,i),datavvi(:,j),cctype);
                cci(j,:)=ccij;
            end
            ccv{i}=cci;
            distv{i}=disti;
